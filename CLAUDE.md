...
<!-- /orca auto-applied -->
<!-- orca auto-applied (finding c984fdad-945a-48e4-8f08-2db081bad302) -->
When implementing or modifying the periodic audit trigger (currently in `apps/server/src/routes/audit.ts`), the cron/interval handler must: (1) call `resyncImplementationAudit` first, then (2) query for all rows where `status = 'unaudited'` OR `recipeStale = true`, and (3) call `runAuditRowAgent` on each. A handler that only calls resyncImplementationAudit is incomplete. Additionally, in the PATCH `/:id` route, when the incoming `body.status === 'unaudited'`, the update object must explicitly set `lastReviewedAt: null`, `lastAuditedAt: null`, and `auditVerdict: null` — do not stamp a new Date().
<!-- /orca auto-applied -->
