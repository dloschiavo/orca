...
description: >
  ...
  (flag/sentiment/resolved/date/source-specific), paired flag and
...
- Page: **`/platform/chat`** — directory-style, platform-level per `admin-routing/SKILL.md` § The Three Authenticated Trees. Sibling pages: `/{org-slug}/users` (org-scoped), `/admin/orgs` (staff org-CRUD), `/platform/prompts` (platform). Never flat-file as `admin-chat.tsx` or `platform-chat.tsx`.
- Sidebar label: **"Chat"**.
...
Standard admin page shell (see `admin-design-system`). Header row contains:

1. **Tab strip** — left-aligned, one button per tab. Active tab gets the canonical active-tab styling from the design system.
...
Both flag and sentiment badges use **paired status classes** (`bg-{color}-100` + `text-{color}-700`). No hex, no inline styles. See `admin-design-system` for the canonical Badge component.
...
7.1 Resolved state: button using paired state colors (`bg-emerald-900` / `text-emerald-400`), label `"resolved"` (lowercase).

7.2 Open state: paired warn colors (`bg-orange-900` / `text-orange-400`), label `"open"`.
...
14.4 Row detail panel: bounded height with internal scroll (`max-h-80 overflow-auto`) inside the canonical dark card surface. **Why:** unbounded height pushes subsequent rows off-screen on documents with long `raw` payloads.
...
- **Hex colors or inline styles for badges** — paired status classes only. The flag/sentiment color tables in this recipe are the source of truth across both tabs.
