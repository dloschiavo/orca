...
**Component bindings** (per `admin-design-system`):
- Top banner — full-width alert (per the design system); sticky position uses `position: fixed; top: 0; left: 0; right: 0; z-index: 9999` (the canonical pattern, not `absolute` inside an overflow container)
- Cards — the design system's card surface with the paired status badge for the check status (red/yellow/blue/green per the design system's badge palette)
- "Recheck now" buttons — the design system's button variant `light` for per-card, variant `danger` for the banner-level button
