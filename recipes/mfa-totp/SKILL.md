...
dependencies:
  requires: [otp-auth, admin-routing]
  capabilities:
...
Events: `enroll_started`, `enroll_completed`, `verify_success`, `verify_failure`, `recovery_used`, `recovery_regenerated`, `self_disabled`, `peer_reset`, `support_reset_requested`, `support_reset_applied`, `policy_changed`. Never delete rows. This collection is read by `admin-feed`'s `security.notable` producer.
...
Add a **Security** section to the org administration surface in the slugged workspace (`/{org-slug}/admin`, defined by `admin-routing`; if the project has a dedicated org-settings page, put it there). Only render the toggle to org `owner` members; for non-owners, show the current state as read-only.
...
The `security.notable` producer in `admin-feed` reads this collection for: failure bursts (≥10 `verify_failure` in 1h for one user), unusual peer-reset patterns, and any `support_reset_requested` for visibility to the platform team.
