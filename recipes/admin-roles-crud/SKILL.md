...
**Canonical path scheme.** The roles editor and permissions catalog are platform-wide concerns (every org shares one role catalog), so they live under `/platform/**` per `admin-routing/SKILL.md` § The Three Authenticated Trees. Pages use directory-style routing (`app/platform/{name}/page.tsx`, or `app/(app)/platform/{name}/index.tsx` on Expo Router) with single-noun sidebar labels — never flat-file as `platform-roles.tsx`. See sibling skills for surrounding context:

| Path | Sidebar label | Recipe |
|---|---|---|
| `/{org-slug}/users` | Users | `admin-routing` (tenancy) / `admin-user-crud` |
| `/admin/orgs` | Organizations | `admin-routing` (tenancy) |
