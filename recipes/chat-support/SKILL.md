...
5. Input bar — TextInput + send button (paper-plane icon). **MUST use the canonical form input styling from the design system — see § Input Bar Styling below.**
...
**Why this matters:** the chat drawer is the most prominent piece of UI in the app. When its input doesn't match the inputs on every other screen, the whole product feels stitched together from different apps. See `admin-design-system/SKILL.md` (the "Reinventing the chat input" anti-pattern) for the full design-system rationale.
