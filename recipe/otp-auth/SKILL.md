...
- The OTP request/verify endpoints are not bypassed — real login still works in dev and creates a real session, after which `user` is non-null and the bypass flag is irrelevant.
...
- **Bypassing the OTP request/verify endpoints.** They must run their normal flow so the real login path is exercised during dev. The bypass is for skipping the gate, not for short-circuiting OTP.
...
**This pattern is intentionally not behind `LOCALHOST_AUTH_BYPASS=true`.** That bypass skips the OTP flow entirely. The dev console log is for the case where you *want* to test the real flow (e.g. you're debugging the IP check, the rate limiter, or the redirect-survives-promotion path) without needing prod email creds.
