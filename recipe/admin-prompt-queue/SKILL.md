...
Prompt templates and the queue surface are platform-wide concerns (every org reads from the same prompt catalog), so this recipe's pages live under `/platform/**` per `admin-routing/SKILL.md` § Two trees. Pages use directory-style routing with single-noun sidebar labels:

| Path | Sidebar label | Tree |
|---|---|---|
| `/admin/users` | Users | org-level — `admin-user-crud` |
| `/admin/orgs` | Organizations | org-level — `multi-tenant` |
